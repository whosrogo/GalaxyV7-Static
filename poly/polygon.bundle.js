	...
	E = {
		4322: function (A) {
			...
			function B(A, B) {
				var Q,
					...
					D =
						...
						(C = ''),
						(E = ''),
						(i = Q.split('=')).length > 1 ? ((C = i.shift()), (E = i.join('='))) : (E = Q),
						{ name: C, value: E }),
					t = D.name,
					e = D.value;
			...
			function Q(A, Q) {
				...
				if (A.headers)
					if ('function' == typeof A.headers.getSetCookie) A = A.headers.getSetCookie();
		...
		336: function (A, I, g) {
			...
			class r {
				...
				constructor(A) {
					...
					if (e.iswindow) {
						try {
							B.pX in A.parent && (this.box = A.parent[B.pX].box);
						} catch {}
						try {
							B.pX in A.top && (this.box = A.top[B.pX].box);
						} catch {}
						try {
							A.opener && B.pX in A.opener && (this.box = A.opener[B.pX].box);
						} catch {}
						this.box || (w.warn('Creating SingletonBox'), (this.box = new s.SingletonBox(this)));
					} else this.box = new s.SingletonBox(this);
							...
							store: new Proxy(
								{},
								{
									get: (A, g) => {
										...
										if (!C) return;
										let E = I.natives.call('Object.getOwnPropertyDescriptor', null, C, Q);
										return ((A[g] = E), A[g]);
									}
								}
							),
					...
					((this.meta = {
						...
						get base() {
							if (e.iswindow) {
								...
								if (A) {
									...
									if (!g) return I.url;
									let B = g.indexOf('#');
									if (!(g = g.substring(0, -1 === B ? void 0 : B))) return I.url;
						...
						get topFrameName() {
							if (!e.iswindow) throw Error('topFrameName was called from a worker?');
							let A = I.global;
							if (A.parent.window == A.window) return null;
							for (; A.parent.window !== A.window && A.parent.window[B.pX]; ) A = A.parent.window;
						...
						get parentFrameName() {
							if (!e.iswindow) throw Error('parentFrameName was called from a worker?');
					...
					for (let g of A.keys()) {
						...
						g.endsWith('.ts') &&
							((g.startsWith('./dom/') && 'window' in this.global) ||
								(g.startsWith('./worker/') && 'WorkerGlobalScope' in this.global) ||
								g.startsWith('./shared/')) &&
							I.push(B);
					}
				...
				set url(A) {
					...
					(this.frame && this.frame.dispatchEvent(I),
						I.defaultPrevented || (this.global.location.href = (0, o.Oy)(I.url, this.meta)));
				}
				Proxy(A, I) {
					if (Array.isArray(A)) {
						...
						(C.construct = function (A, I, B) {
							let Q,
								...
								E = {
									...
									call: () => ((C = !0), (Q = Reflect.construct(E.fn, E.args, E.newTarget)))
								};
							return (g.construct(E), C) ? Q : Reflect.construct(E.fn, E.args, E.newTarget);
						}),
						g.apply &&
							(C.apply = (A, I, B) => {
								let Q,
									...
									E = {
										...
										call: () => ((C = !0), (Q = Reflect.apply(E.fn, E.this, E.args)))
									},
									i = Error.prepareStackTrace;
								Error.prepareStackTrace = function (A, I) {
									if (
										I[0].getFileName() &&
										!I[0].getFileName().startsWith(location.origin + D.$W.prefix)
									)
								...
								return ((Error.prepareStackTrace = i), C) ? Q : Reflect.apply(E.fn, E.this, E.args);
							}),
				...
				Trap(A, I) {
					...
					let g = A.split('.'),
						...
						Q = g.reduce((A, I) => A?.[I], this.global);
					if (!Q) return;
					let C = this.natives.call('Object.getOwnPropertyDescriptor', null, Q, B);
					return ((this.descriptors.store[A] = C), this.RawTrap(Q, B, I));
				}
				RawTrap(A, I, g) {
					if (!A || !I || !Reflect.has(A, I)) return;
					let B = this.natives.call('Object.getOwnPropertyDescriptor', null, A, I),
						Q = {
							this: null,
							get: function () {
			...
			function B(A, I) {
				(A.Trap('Element.prototype.attributes', {
										...
										: 'getNamedItem' === B
											...
											: 'getNamedItemNS' === B
												...
												: B in NamedNodeMap.prototype && 'function' == typeof C
													? new Proxy(C, {
															apply: (A, B, Q) =>
																B === g ? Reflect.apply(A, I, Q) : Reflect.apply(A, B, Q)
														})
													: ('string' != typeof B && 'number' != typeof B) || isNaN(Number(B))
														? this.has(A, B)
															? C
															: void 0
														: I[Object.keys(g)[B]];
					...
					A.Trap(['Attr.prototype.value', 'Attr.prototype.nodeValue'], {
						get: (A) =>
							A.this?.ownerElement ? A.this.ownerElement.getAttribute(A.this.name) : A.get(),
						set: (A, I) =>
							A.this?.ownerElement ? A.this.ownerElement.setAttribute(A.this.name, I) : A.set(I)
					}));
			}
		...
		5351: function (A, I, g) {
			...
			function Q(A, I) {
				let g = String;
				(A.Proxy(['Document.prototype.querySelector', 'Document.prototype.querySelectorAll'], {
					apply(A) {
						A.args[0] = g(A.args[0]).replace(
					...
					A.Trap('Document.prototype.referrer', {
						get: () => A.url.toString()
					}),
		...
		7828: function (A, I, g) {
			...
			function s(A, I) {
				let g = {
					...
					s = [I.HTMLAnchorElement.prototype, I.HTMLAreaElement.prototype],
					a = [
						A.natives.call(
				...
				for (let I of Object.keys(g))
					for (let B of g[I]) {
						let g = A.natives.call('Object.getOwnPropertyDescriptor', null, B.prototype, I);
						Object.defineProperty(B.prototype, I, {
							get() {
								return ['src', 'data', 'href', 'action', 'formaction'].includes(I)
				...
				(A.Trap('Node.prototype.baseURI', {
							...
							let [g, Q] = I.args,
								C = B.V.find((A) => {
									...
									return (
										!!B &&
										('*' === B ||
											('function' != typeof B && B.includes(I.this.tagName.toLowerCase())))
									);
								});
							if (C) {
								let B = C.fn(Q, A.meta, A.cookieStore);
								if (null == B) {
									(A.natives.call('Element.prototype.removeAttribute', I.this, g),
										I.return(void 0));
									return;
								}
							...
							let [g, Q, C] = I.args,
								E = B.V.find((A) => {
									...
									return (
										!!g &&
										('*' === g ||
											('function' != typeof g && g.includes(I.this.tagName.toLowerCase())))
									);
								});
							E &&
					...
					A.Trap('Element.prototype.innerHTML', {
						...
						get(g) {
							...
							return g.this instanceof I.HTMLStyleElement ? g.get() : (0, C.nK)(g.get());
						}
					}),
					A.Trap('Node.prototype.textContent', {
						set(g, B) {
							if (g.this instanceof I.HTMLScriptElement) {
								let I = (0, E.o)(B, '(anonymous script element)', A.meta);
								return (
									A.natives.call(
										'Element.prototype.setAttribute',
										g.this,
							...
							return g.this instanceof I.HTMLStyleElement ? g.set((0, Q.s)(B, A.meta)) : g.set(B);
						},
						get(g) {
							if (g.this instanceof I.HTMLScriptElement) {
								let I = A.natives.call(
							...
							return g.this instanceof I.HTMLStyleElement ? (0, Q.f)(g.get()) : g.get();
						}
					}),
					...
					A.Trap('Text.prototype.wholeText', {
						get: (A) => (A.this.parentElement?.tagName === 'STYLE' ? (0, Q.f)(A.get()) : A.get()),
						set: (I, g) =>
							I.this.parentElement?.tagName === 'STYLE' ? I.set((0, Q.s)(g, A.meta)) : I.set(g)
					}),
					A.Trap(
						[
		...
		9804: function (A, I, g) {
			...
			var B = g(1472),
				...
				C = g(2794);
			function E(A, I) {
				A.Proxy(['History.prototype.pushState', 'History.prototype.replaceState'], {
					apply(I) {
						((I.args[2] || '' === I.args[2]) && (I.args[2] = (0, B.Oy)(I.args[2], A.meta)),
							I.call());
						let {
		...
		6286: function (A, I, g) {
			...
			function C(A, I) {
				(A.Trap('PerformanceEntry.prototype.name', {
					get(A) {
						let I = A.get();
						return I && I.startsWith(location.origin + Q.$W.prefix) ? (0, B.v2)(I) : I;
					}
				}),
					A.Proxy(
						[
						...
						],
						{
							apply(A) {
								...
								return A.return(
									I.filter((A) => {
										for (let I of Object.values(Q.$W.files))
											if (A.name.startsWith(location.origin + I)) return !1;
										return !0;
									})
								);
							}
				...
				(A.Proxy('EventTarget.prototype.addEventListener', {
					...
					A.Proxy('ServiceWorkerContainer.prototype.register', {
						apply(I) {
							...
							(Object.setPrototypeOf(B, self.ServiceWorkerRegistration.prototype),
								(B.constructor = I.fn));
				...
				let g = {
					get(I, g) {
						switch (g) {
							...
							case 'clear':
								return () => {
									for (let g in Object.keys(I)) g.startsWith(A.url.host) && I.removeItem(g);
								};
							case 'key':
								return (g) => {
									let B = Object.keys(I).filter((I) => I.startsWith(A.url.host));
									return I.getItem(B[g]);
								};
							case 'length':
								return Object.keys(I).filter((I) => I.startsWith(A.url.host)).length;
							default:
								if (g in Object.prototype || 'symbol' == typeof g) return Reflect.get(I, g);
					...
					ownKeys: (I) =>
						Reflect.ownKeys(I)
							...
							.map((I) => ('string' == typeof I ? I.substring(A.url.host.length + 1) : I)),
					...
					defineProperty: (I, g, B) => (I.setItem(A.url.host + '@' + g, B.value), !0)
				};
				I.localStorage;
		...
		1323: function (A, I, g) {
			...
			var B = g(37),
				...
				i = g(8409),
				o = g(8665).A;
			let D = 'window' in globalThis && window instanceof Window,
				t = 'WorkerGlobalScope' in globalThis,
		...
		3696: function (A, I, g) {
			...
			function E(A, I) {
				...
				for (let g of [
					...
					if (!Q) continue;
					let C = { configurable: !1, enumerable: !0 };
				...
				return (
					...
					I.location.assign &&
						(E.assign = new Proxy(I.location.assign, {
							apply(g, C, E) {
								((E[0] = (0, Q.Oy)(E[0], A.meta)), Reflect.apply(g, I.location, E));
		...
		4634: function (A, I, g) {
			...
			function Q(A) {
				(A.Proxy('URL.createObjectURL', {
					apply(I) {
						let g = I.call();
						g.startsWith('blob:') ? I.return((0, B.IP)(g, A.meta)) : I.return(g);
					}
				}),
					...
					A.Proxy('Cache.prototype.addAll', {
						apply(I) {
							for (let g = 0; g < I.args[0].length; g++)
								('string' == typeof I.args[0][g] || I.args[0][g] instanceof URL) &&
			...
			function C(A, I = []) {
				switch (typeof A) {
					case 'string':
						break;
					case 'object':
						if (A && A[Symbol.iterator] && 'function' == typeof A[Symbol.iterator])
							for (let g in A) {
		...
		591: function (A, I, g) {
			...
			function C(A, I) {
				Object.defineProperty(I, B.$W.globals.rewritefn, {
					value: function (I) {
						return 'string' != typeof I ? I : (0, Q.o)(I, '(direct eval proxy)', A.meta);
					},
					writable: !1,
					configurable: !1
				});
										...
										get(A, g, B) {
											...
											return g in I
												...
												: 'function' == typeof Q
													? new Proxy(Q, {
															apply: (A, I, g) =>
																I === B ? Reflect.apply(A, E, g) : Reflect.apply(A, I, g)
														})
													: Q;
										},
				...
				(A.Proxy('EventTarget.prototype.addEventListener', {
					...
					A.Proxy('EventTarget.prototype.removeEventListener', {
						apply(I) {
							...
							if (!g) return;
							let B = g.findIndex((A) => A.event === I.args[0] && A.originalCallback === I.args[1]);
							if (-1 === B) return;
							let Q = g.splice(B, 1);
							(A.eventcallbacks.set(I.this, g), (I.args[1] = Q[0].proxiedCallback));
						}
					}));
				...
				o))
					for (let I of Reflect.ownKeys(Q))
						if ('string' == typeof I && I.startsWith('on') && g[I.slice(2)]) {
							let g = A.natives.call('Object.getOwnPropertyDescriptor', null, Q, I);
		...
		249: function (A, I, g) {
			...
			function C(A, I) {
				...
				let B = A.natives.call('eval', null, '(function () {})').constructor,
					C = A.natives.call('eval', null, '(async function () {})').constructor,
					E = A.natives.call('eval', null, '(function* () {})').constructor,
					i = A.natives.call('eval', null, '(async function* () {})').constructor;
		...
		2468: function (A, I, g) {
			...
			var B = g(37),
				Q = g(1472);
			function C(A, I) {
				let g = A.natives.call('Function', null, 'url', 'return import(url)');
				(Object.defineProperty(I, B.$W.globals.importfn, {
					value: function (I, B) {
						let C = new URL(B, I).href;
						return B.includes(':') || B.startsWith('/') || B.startsWith('.') || B.startsWith('..')
							? g(`${(0, Q.Oy)(C, A.meta)}?type=module`)
							: g(B);
					},
					writable: !1,
					configurable: !1,
				...
				A.Proxy('StorageManager.prototype.getDirectory', {
					apply(I) {
						...
						I.return(
							(async () => {
								let I = await g,
									B = await I.getDirectoryHandle(`${A.url.origin.replace(/\/|\s|\./g, '-')}`, {
										create: !0
									});
								return (Object.defineProperty(B, 'name', { value: '', writable: !1 }), B);
							})()
						);
					}
				});
				...
				(A.Proxy('WebSocket', {
								...
								('string' == typeof I ||
									('byteLength' in I
										...
										: 'arrayBuffer' in I &&
											...
											Object.setPrototypeOf((I = await I.arrayBuffer()), ArrayBuffer.prototype)),
									i(
					...
					A.Proxy('WebSocketStream.prototype.close', {
						apply(A) {
							let I = B.get(A.this);
							return A.args[0]
								? (void 0 === A.args[0].closeCode && (A.args[0].closeCode = 1e3),
									void 0 === A.args[0].reason && (A.args[0].reason = ''),
									A.return(I.barews.close(A.args[0].closeCode, A.args[0].reason)))
				...
				(A.Proxy('XMLHttpRequest.prototype.open', {
					...
					A.Proxy('XMLHttpRequest.prototype.send', {
						apply(I) {
							...
							for (; 0 === o.getUint8(0); )
								if (performance.now() - D > 1e3) throw Error('xhr timeout');
							let t = o.getUint16(1),
								e = o.getUint32(3),
								s = new Uint8Array(e);
							...
							(A.RawTrap(I.this, 'status', { get: () => t }),
								A.RawTrap(I.this, 'responseText', { get: () => h }),
								A.RawTrap(I.this, 'response', {
									get: () => ('arraybuffer' === I.this.responseType ? r.buffer : h)
								}),
		...
		7418: function (A, I, g) {
			...
			var B = g(1478);
			function Q(A, I) {
				A.Proxy(['setTimeout', 'setInterval'], {
					apply(I) {
						I.args.length > 0 &&
							'string' == typeof I.args[0] &&
							(I.args[0] = (0, B.o)(I.args[0], '(setTimeout string eval)', A.meta));
					}
				});
			}
		},
							...
							for (let A = 0; A < i; A++) {
								...
								if (((o += 1), 0 == g)) E.push({ type: g, start: A, size: I });
									...
									let i = A + I,
										D = Q.getUint32(o, !0);
									o += 4;
									let t = C.decode(B.subarray(o, o + D));
									E.push({ type: g, start: A, end: i, str: t });
								}
							}
						...
						apply(I) {
							(performance.now(),
								(function (A, I) {
									...
									if (!t) return (console.warn('failed to get rewrites for tag', Q), I.return(g));
									let e = 0;
									for (; e < t.length; )
										if (t[e].start < o) e++;
										else break;
									...
									let a = t.slice(e, s),
										...
										r = 0;
									for (let A of a)
										if (((w += g.slice(r, A.start - o)), 0 === A.type)) r = A.start + A.size - o;
										else if (1 === A.type) ((w += A.str), (r = A.end - o));
										else throw 'unreachable';
									((w += g.slice(r)), (w = w.replace(`${C}${i} ${Q}*/`, '')), I.return(w));
								})(A, I));
						}
					}));
			}
				...
				(A.Proxy('Worker', {
					construct(I) {
						((I.args[0] = (0, Q.Oy)(I.args[0], A.meta) + '?dest=worker'),
							I.args[1] && 'module' === I.args[1].type && (I.args[0] += '&type=module'));
						let g = I.call(),
							C = new B.DD();
						(async () => {
						...
						construct(I) {
							((I.args[0] = (0, Q.Oy)(I.args[0], A.meta) + '?dest=sharedworker'),
								...
								I.args[1] &&
									...
									I.args[1].name && (I.args[1].name = `${A.url.origin}@${I.args[1].name}`)));
							let g = I.call(),
								C = new B.DD();
							(async () => {
								let I = await C.getInnerPort();
					...
					A.Proxy('Worklet.prototype.addModule', {
						apply(I) {
							I.args[0] && (I.args[0] = (0, Q.Oy)(I.args[0], A.meta) + '?dest=worklet');
						}
					}));
			}
		},
		581: function (A, I, g) {
			(g.r(I), g.d(I, { createWrapFn: () => i, default: () => D, order: () => o }));
			var B = g(1323),
				Q = g(2794),
				C = g(37),
				E = g(591);
			...
			function D(A, I) {
				(Object.defineProperty(I, C.$W.globals.wrapfn, {
					...
					Object.defineProperty(I, C.$W.globals.wrappropertyfn, {
						value: function (A) {
							return 'location' === A || 'parent' === A || 'top' === A || 'eval' === A
					...
					Object.defineProperty(I.Object.prototype, C.$W.globals.wrappropertybase + 'location', {
						get: function () {
							return this === I || this === I.document ? A.locationProxy : this.location;
						},
						set(g) {
					...
					Object.defineProperty(I.Object.prototype, C.$W.globals.wrappropertybase + 'parent', {
						get: function () {
					...
					Object.defineProperty(I.Object.prototype, C.$W.globals.wrappropertybase + 'top', {
						get: function () {
					...
					Object.defineProperty(I.Object.prototype, C.$W.globals.wrappropertybase + 'eval', {
						get: function () {
					...
					(I.$cinnaitize = function (A) {
						return (
							location,
							B.iswindow && I.top,
							'string' == typeof A && A.includes('poly'),
					...
					Object.defineProperty(I, C.$W.globals.trysetfn, {
						value: function (g, B, Q) {
							return g instanceof I.Location && ((A.locationProxy.href = Q), !0);
						},
						writable: !1,
						configurable: !1
					}));
		...
		3402: function (A, I, g) {
			...
			class o extends EventTarget {
				...
				constructor(A) {
					...
					let I = (A, g) => {
							for (let B in g)
								g[B] instanceof Object && B in A && Object.assign(g[B], I(A[B], g[B]));
				...
				encodeUrl(A) {
					...
					return ((A.hash = ''), B.$W.prefix + (0, B.hD)(A.href) + (I ? '#' + I : ''));
				}
				decodeUrl(A) {
					A instanceof URL && (A = A.toString());
					...
					let A = await (0, C.P2)('$cinnabar', 1, {
						upgrade(A) {
							(A.objectStoreNames.contains('config') || A.createObjectStore('config'),
								...
								A.objectStoreNames.contains('redirectTrackers') ||
									A.createObjectStore('redirectTrackers'),
								A.objectStoreNames.contains('referrerPolicies') ||
									A.createObjectStore('referrerPolicies'),
								A.objectStoreNames.contains('publicSuffixList') ||
									A.createObjectStore('publicSuffixList'));
						}
					});
					return ((this.db = A), await this.#A(), A);
				}
				async #A() {
					this.db ? await this.db.put('config', B.$W, 'config') : console.error('Store not ready!');
				}
					...
					fmt: function (A, I, ...g) {
						...
						Error.prepareStackTrace = (A, I) => {
							...
							for (let A = 1; A < Math.min(2, I.length); A++)
								I[A].getFunctionName() && (g += `${I[A].getFunctionName()} -> ` + g);
  ...
  `,
				...
				getCookies(A, I) {
					...
					for (let C of B) {
						...
						(!C.secure || 'https:' === A.protocol) &&
							...
							(!C.domain.startsWith('.') || A.hostname.endsWith(C.domain.slice(1))) &&
							Q.push(C);
					}
		...
		2393: function (A, I, g) {
			...
			let E = [
				...
				{
					fn: (A, I) => (A.startsWith('blob:') ? (0, C.$n)(A) : (0, C.Oy)(A, I)),
					src: ['video', 'audio']
				},
		...
		37: function (A, I, g) {
			...
			let e = Function;
			function s() {
				((B = e(`return ${C.codec.encode}`)()), (Q = e(`return ${C.codec.decode}`)()));
			}
			function a(A, I) {
				let g = C.flags[A];
		...
		2614: function (A, I, g) {
			...
			function E(A, I, g) {
				return (I = (I = new String(I).toString()).replace(/url\(['"]?(.+?)['"]?\)/gm, (I, Q) => {
					let C = 'rewrite' === A ? (0, B.Oy)(Q.trim(), g) : (0, B.v2)(Q.trim());
					...
					(I, Q) =>
						I.replace(
							Q,
							Q.replace(/^(url\(['"]?|['"]|)(.+?)(['"]|['"]?\)|)$/gm, (I, Q, C, E) => {
								if (Q.startsWith('url')) return I;
								let i = 'rewrite' === A ? (0, B.Oy)(C.trim(), g) : (0, B.v2)(C.trim());
								return `${Q}${i}${E}`;
							})
						)
				);
		...
		4435: function (A, I, g) {
			...
			async function o(A, I, g, o) {
				let D = {};
				for (let I in A) D[I.toLowerCase()] = A[I];
				for (let A of C) delete D[A];
				for (let A of E) D[A] && (D[A] = (0, B.Oy)(D[A]?.toString(), I));
				if (
					('string' == typeof D.link
						? (D.link = i(D.link, I))
						: Array.isArray(D.link) && (D.link = D.link.map((A) => i(A, I))),
					'string' == typeof D.referer)
				) {
					let A = new URL(D.referer),
				...
				return (
					'string' == typeof D['sec-fetch-dest'] &&
						'' === D['sec-fetch-dest'] &&
						(D['sec-fetch-dest'] = 'empty'),
					'string' == typeof D['sec-fetch-site'] &&
						'none' !== D['sec-fetch-site'] &&
						('string' == typeof D.referer
							? (D['sec-fetch-site'] = await (0, Q.ps)(I, new URL(D.referer), g))
							: (console.warn(
									"Missing referrer header; can't rewrite sec-fetch-site properly. Falling back to unsafe deletion."
								),
								delete D['sec-fetch-site'])),
					D
				);
			}
		},
	...
	`,
			...
			function w(A, I, g, D = !1) {
				let r = performance.now(),
					h = (function (A, I, g, D = !1) {
						...
						if (
							...
							(function A(I, g, B) {
								...
								if (
									...
									'script' === I.name && 'importmap' === I.attribs.type && void 0 !== I.children[0])
								) {
									...
									try {
										...
										if (g.imports)
											for (let A in g.imports) {
												let I = g.imports[A];
												'string' == typeof I && ((I = (0, E.Oy)(I, B)), (g.imports[A] = I));
											}
								...
								if (
									'script' === I.name &&
									/(application|text)\/javascript|module|undefined/.test(I.attribs.type) &&
									void 0 !== I.children[0]
								) {
									let A = I.children[0].data,
								...
								if ('meta' === I.name && void 0 !== I.attribs['http-equiv']) {
									if ('content-security-policy' === I.attribs['http-equiv'].toLowerCase())
								...
								if (I.childNodes)
									for (let Q in I.childNodes) I.childNodes[Q] = A(I.childNodes[Q], g, B);
								return I;
							})(e.root, I, g),
							D)
						) {
							...
							(A || ((A = new Q.Hg('head', {}, [])), e.root.children.unshift(A)),
								A.children.unshift(...s(I, (A) => new Q.Hg('script', { src: A }))));
						}
						return (0, C.A)(e.root, {
							encodeEntities: 'utf8',
							decodeEntities: !1
				...
				try {
					...
					if ((0, B.U5)('sourcemaps', g.base)) {
						...
						if (A) A(Array.from(Q.map), Q.tag);
							...
							let A = `${B.$W.globals.pushsourcemapfn}([${Q.map.join(',')}], "${Q.tag}");`,
								I = /^\s*(['"])use strict\1;?/;
							i = I.test(i)
								? i.replace(
										I,
...
${i}`;
		...
		1472: function (A, I, g) {
			...
			function o(A, I) {
				if ((A instanceof URL && (A = A.toString()), A.startsWith('javascript:')))
					return 'javascript:' + (0, Q.o)(A.slice(11), '(javascript: url)', I);
				{
					...
					let g = I.base.href;
					g.startsWith('about:') && (g = D(self.location.href));
					let Q = C(A, g);
					if (!Q) return A;
					let E = (0, B.hD)(Q.hash.slice(1));
					return (
						(Q.hash = ''),
						location.origin + B.$W.prefix + (0, B.hD)(Q.href) + (E ? '#' + E : '')
					);
				}
			}
			function D(A) {
				...
				if (A.startsWith('javascript:')) return A;
				{
					...
					if (A.startsWith(I + 'blob:') || A.startsWith(I + 'data:')) return A.substring(I.length);
					if (A.startsWith('mailto:') || A.startsWith('about:')) return A;
					let g = C(A);
					if (!g) return A;
					let Q = (0, B.P_)(g.hash.slice(1));
					return ((g.hash = ''), (0, B.P_)(g.href.slice(I.length) + (Q ? '#' + Q : '')));
				}
			}
		},
		1561: function (A, I, g) {
				...
				return (
					-1 === g
						? ((0, C.U5)('rewriterLogs', A.base) &&
								console.log(`creating new rewriter, ${e} rewriters made already`),
							(I = {
								rewriter: new Q.LW({
									config: C.$W,
									shared: {
										rewrite: {
											...
											getHtmlInjectCode(A, I) {
												let g = (0, t.Uk)(A, (A) => `<script src="${A}"></script>`).join('');
							...
							w.push(I))
						: ((0, C.U5)('rewriterLogs', A.base) &&
								console.log(`using cached rewriter ${g} from list of ${e} rewriters`),
							(I = w[g])),
					(I.inUse = !0),
...
`);
		...
		6684: function (A, I, g) {
			...
			async function E(A) {
				let I = await C();
				return (await I.get('redirectTrackers', A)) || null;
			}
			async function i(A, I) {
				let g = await C();
				await g.put('redirectTrackers', I, A);
			}
			async function o(A) {
				let I = await C();
				await I.delete('redirectTrackers', A);
			}
			async function D(A, I, g) {
				(await E(A)) ||
					(await i(A, {
						originalReferrer: I || '',
			...
			async function t(A, I, g) {
				let B = await E(A);
				B && (await o(A), g && (B.referrerPolicy = g), await i(I, B));
			}
			async function e(A, I) {
				let g = await E(A);
				if (!g) return I;
				let B = Q[g.mostRestrictiveSite];
				return (Q[I] ?? 0) > B
					? ((g.mostRestrictiveSite = I), await i(A, g), I)
			...
			async function a() {
				let A = Date.now(),
					I = (await C()).transaction('redirectTrackers', 'readwrite');
				for await (let g of I.store) A - g.value.chainStarted > 36e5 && g.delete();
				await I.done;
			}
			async function w(A, I, g) {
				let B = await C();
				await B.put('referrerPolicies', { policy: I, referrer: g }, A);
			}
			async function r(A) {
				let I = await C();
				return (await I.get('referrerPolicies', A)) || null;
			}
		},
		2416: function (A, I, g) {
			g.d(I, {
		...
		8228: function (A, I, g) {
			...
			async function E() {
				let A = await C();
				return (await A.get('publicSuffixList', Q)) || null;
			}
			async function i(A) {
				let I = await C();
				await I.put('publicSuffixList', { data: A, expiry: Date.now() + 36e5 }, Q);
			}
			async function o(A, I, g) {
				return I
					? A.origin.origin === I.origin
						? 'same-origin'
						: (await D(A.origin, I, g))
							? 'same-site'
							: 'cross-site'
					: 'none';
			}
			...
			async function e(A) {
				...
				try {
					I = await A.fetch('https://publicsuffix.org/list/public_suffix_list.dat');
				} catch (A) {
        ...
        `;
			...
			function Q(A, I) {
				...
				return (
					crossOriginIsolated && (g['Cross-Origin-Embedder-Policy'] = 'require-corp'),
...
self.WASM = '${g}';`),
					...
					for (let [A, I] of [...i.searchParams.entries()]) {
						switch (A) {
							...
							case 'parentFrame':
								...
								break;
							default:
								(w.warn(`${i.href} extraneous query parameter ${A}. Assuming <form> element`),
									(e[A] = I));
						}
					...
					if (
						...
						((B.finalURL = g.startsWith('blob:') ? g : '(data url)'),
							B.body && (I = await S(B, a, A.destination, t, this.cookieStore)));
					...
					if (I && new URL(I.url).pathname.startsWith(D.$W.prefix)) {
					...
					if (
						...
						A.referrer !== location.origin + D.$W.prefix + 'no-referrer'
					) {
						let I = A.referrer,
							g = await self.clients.matchAll({ type: 'window' });
						for (; I; ) {
							if (!I.includes(D.$W.prefix)) {
								N = !0;
								break;
							}
					...
					N
						? (h.set('Sec-Fetch-Dest', 'document'), h.set('Sec-Fetch-Mode', 'navigate'))
						: (h.set('Sec-Fetch-Dest', A.destination || 'empty'), h.set('Sec-Fetch-Mode', A.mode));
					let n = 'none';
					if (
						A.referrer &&
						'' !== A.referrer &&
						'no-referrer' !== A.referrer &&
						A.referrer !== location.origin + D.$W.prefix + 'no-referrer' &&
						A.referrer.includes(D.$W.prefix)
					) {
					...
					(await (0, C.rj)(s.toString(), A.referrer ? (0, E.v2)(A.referrer) : null, n),
			...
			async function c(A, I, g, B, i, o, e, s, a, w, h) {
				let c,
					...
					N = await (0, t.l)(o.rawHeaders, I, a, { get: C.Yq, set: C.pL });
				if (
					(y && N['referrer-policy'] && h && (await (0, C.pL)(A.href, N['referrer-policy'], h)),
					r(o))
				) {
				...
				if (
					...
					if ((0, D.U5)('interceptDownloads', A)) {
						...
						let B = N['content-length'],
							Q = await clients.matchAll({});
						if ((Q = Q.filter((A) => !A.url.includes(D.$W.prefix))).length < 1)
				...
				(o.body && !r(o) && (c = await S(o, I, B, g, e)),
					'text/event-stream' === N.accept && (N['content-type'] = 'text/event-stream'),
					delete N['permissions-policy'],
					crossOriginIsolated &&
						['document', 'iframe', 'worker', 'sharedworker', 'style', 'script'].includes(B) &&
			...
			async function S(A, I, g, B, Q) {
				switch (g) {
					...
					case 'document':
						...
						return A.body;
					case 'script':
						return (0, i.o)(new Uint8Array(await A.arrayBuffer()), A.finalURL, I, 'module' === B);
					case 'style':
						return (0, s.s)(await A.text(), I);
					case 'sharedworker':
					case 'worker':
						return (0, a.i)(new Uint8Array(await A.arrayBuffer()), B, A.finalURL, I);
					default:
						return A.body;
				}
			}
		...
		236: function (A, I, g) {
			...
			function t(A) {
				...
				return (i.call(A, { message: { type: 'ping' }, port: I.port2 }, [I.port2]), g);
			}
			function e(A, I) {
			...
			class w extends EventTarget {
				...
				send(...A) {
					...
					let I = A[0];
					(I.buffer && (I = I.buffer.slice(I.byteOffset, I.byteOffset + I.byteLength)),
						i.call(
							this.channel.port1,
							{ type: 'data', data: I },
		...
		`,
				...
				async setManualTransport(A, I, g) {
					...
					await this.worker.sendMessage({ type: 'set', client: { function: A, args: I } }, g);
				}
							...
							try {
								(A.ready || (await A.init()),
									await (async function (A, I, g) {
										...
										B.body instanceof ReadableStream || B.body instanceof ArrayBuffer
								...
								(A.ready || (await A.init()),
									await (async function (A, I, g) {
										let [B, Q] = g.connect(
											...
											(I) => {
												I instanceof ArrayBuffer
													? i.call(A.websocket.channel, { type: 'message', args: [I] }, [I])
													: i.call(A.websocket.channel, {
															type: 'message',
															args: [I]
														});
										...
										((A.websocket.channel.onmessage = (A) => {
											'data' === A.data.type
												? B(A.data.data)
												: 'close' === A.data.type && Q(A.data.closeCode, A.data.closeReason);
										}),
			...
			class y {
				...
				createWebSocket(A, I = [], g, B) {
					try {
						...
						throw new DOMException(`Faiiled to construct 'WebSocket': The URL '${A}' is invalid.`);
					}
				...
				async fetch(A, I) {
					...
					for (let A = 0; ; A++) {
						...
						((Q.rawHeaders = B.headers), (Q.rawResponse = B), (Q.finalURL = i.toString()));
								...
								case B.vw:
									return (function (I, g) {
										...
										('foreign' === g.xmlMode &&
											...
											I.parent && t.has(I.parent.name) && (g = { ...g, xmlMode: !1 })),
											!g.xmlMode && e.has(I.name) && (g = { ...g, xmlMode: 'foreign' }));
										let E = `<${I.name}`,
											D = (function (A, I) {
												var g;
												if (!A) return;
												let B =
													(null != (g = I.encodeEntities) ? g : I.decodeEntities) === !1
														? i
										...
										return (
											D && (E += ` ${D}`),
											0 === I.children.length &&
											(g.xmlMode ? !1 !== g.selfClosingTags : g.selfClosingTags && o.has(I.name))
												? (g.xmlMode || (E += ' '), (E += '/>'))
												: ((E += '>'),
													I.children.length > 0 && (E += A(I.children, g)),
													(g.xmlMode || !o.has(I.name)) && (E += `</${I.name}>`)),
											E
										);
									})(I, g);
								case B.EY:
									return (function (A, I) {
										...
										return (
											(null != (g = I.encodeEntities) ? g : I.decodeEntities) === !1 ||
												...
												(B =
													I.xmlMode || 'utf8' !== I.encodeEntities ? (0, Q.WY)(B) : (0, Q.X1)(B)),
											B
										);
									})(I, g);
							}
			...
			class E {
				...
				ontext(A) {
					...
					if (I && I.type === B.RJ.Text)
						((I.data += A), this.options.withEndIndices && (I.endIndex = this.parser.endIndex));
					else {
						let I = new Q.EY(A);
				...
				addNode(A) {
					...
					(this.options.withStartIndices && (A.startIndex = this.parser.startIndex),
						this.options.withEndIndices && (A.endIndex = this.parser.endIndex),
						I.children.push(A),
						g && ((A.prev = g), (g.next = A)),
						(A.parent = I),
		...
		6072: function (A, I, g) {
			...
			class s extends D {
				constructor(
					...
					I,
					g = [],
					Q = 'script' === A ? B.RJ.Script : 'style' === A ? B.RJ.Style : B.RJ.Tag
				) {
				...
				get attributes() {
					return Object.keys(this.attribs).map((A) => {
						...
						return {
							...
							namespace: null == (I = this['x-attribsNamespace']) ? void 0 : I[A],
			...
			function a(A, I = !1) {
				...
				if (A.type === B.RJ.Text) g = new E(A.data);
					...
					(B.forEach((A) => (A.parent = Q)),
						...
						A['x-attribsNamespace'] && (Q['x-attribsNamespace'] = { ...A['x-attribsNamespace'] }),
						A['x-attribsPrefix'] && (Q['x-attribsPrefix'] = { ...A['x-attribsPrefix'] }),
						(g = Q));
					...
					let B = I ? w(A.children) : [],
						Q = new e(B);
					(B.forEach((A) => (A.parent = Q)), A['x-mode'] && (Q['x-mode'] = A['x-mode']), (g = Q));
				...
				return (
					...
					null != A.sourceCodeLocation && (g.sourceCodeLocation = A.sourceCodeLocation),
					g
				);
			}
			function w(A) {
				let I = A.map((A) => a(A, !0));
				for (let A = 1; A < I.length; A++) ((I[A].prev = I[A - 1]), (I[A - 1].next = I[A]));
				return I;
			}
		},
		3256: function (A, I, g) {
		...
		2146: function (A, I, g) {
			...
			function E(A) {
				var I;
				return (A >= 55296 && A <= 57343) || A > 1114111 ? 65533 : null != (I = Q.get(A)) ? I : A;
			}
		},
		2990: function (A, I, g) {
			g.d(I, {
			...
			(((B = i || (i = {}))[(B.NUM = 35)] = 'NUM'),
				...
				((C = D || (D = {}))[(C.EntityStart = 0)] = 'EntityStart'),
			...
			class r {
				...
				write(A, I) {
					switch (this.state) {
						case D.EntityStart:
							...
							return ((this.state = D.NamedEntity), this.stateNamedEntity(A, I));
						case D.NumericStart:
				...
				stateNumericStart(A, I) {
					return I >= A.length
						...
						: (32 | A.charCodeAt(I)) === i.LOWER_X
							? ((this.state = D.NumericHex), (this.consumed += 1), this.stateNumericHex(A, I + 1))
							: ((this.state = D.NumericDecimal), this.stateNumericDecimal(A, I));
				}
				addToNumericResult(A, I, g, B) {
					if (I !== g) {
						let Q = g - I;
						((this.result = this.result * Math.pow(B, Q) + Number.parseInt(A.substr(I, Q), B)),
							(this.consumed += Q));
					}
				}
				stateNumericHex(A, I) {
					...
					for (; I < A.length; ) {
						...
						if (
							...
							return (this.addToNumericResult(A, g, I, 16), this.emitNumericEntity(Q, 3));
						I += 1;
					}
				...
				stateNumericDecimal(A, I) {
					...
					for (; I < A.length; ) {
						...
						if (!w(B)) return (this.addToNumericResult(A, g, I, 10), this.emitNumericEntity(B, 2));
						I += 1;
					}
				...
				emitNumericEntity(A, I) {
					...
					return (
						...
						this.errors &&
							(A !== i.SEMI && this.errors.missingSemicolonAfterCharacterReference(),
					...
					null == String.prototype.codePointAt
						? (A, I) =>
								(64512 & A.charCodeAt(I)) == 55296
									? (A.charCodeAt(I) - 55296) * 1024 + A.charCodeAt(I + 1) - 56320 + 65536
									: A.charCodeAt(I)
						: (A, I) => A.codePointAt(I);
			function E(A) {
				let I,
		...
		9496: function () {},
		8466: function (A, I, g) {
			(g.d(I, { Gj: () => i.Gj, WY: () => i.WY, X1: () => i.X1 }), g(2990), g(466));
			var B,
				Q,
				C,
				E,
		...
		4645: function (A, I, g) {
			...
			var B = g(5645),
				Q = g(2990);
			let C = new Set(['input', 'option', 'optgroup', 'select', 'button', 'datalist', 'textarea']),
				E = new Set(['p']),
			...
			class r {
				constructor(A, I = {}) {
					...
					((this.options = I),
						...
						(this.lowerCaseTagNames = null != (g = I.lowerCaseTags) ? g : this.htmlMode),
						(this.lowerCaseAttributeNames =
							null != (Q = I.lowerCaseAttributeNames) ? Q : this.htmlMode),
						(this.recognizeSelfClosing = null != (C = I.recognizeSelfClosing) ? C : !this.htmlMode),
						(this.tokenizer = new (null != (E = I.Tokenizer) ? E : B.A)(this.options, this)),
				...
				endOpenTag(A) {
					...
					((this.startIndex = this.openTagStart),
						this.attribs &&
							(null == (g = (I = this.cbs).onopentag) || g.call(I, this.tagname, this.attribs, A),
							(this.attribs = null)),
						this.cbs.onclosetag &&
				...
				onclosetag(A, I) {
					var g, B, Q, C, E, i, o, D;
					this.endIndex = I;
					let t = this.getSlice(A, I);
					if (
						(this.lowerCaseTagNames && (t = t.toLowerCase()),
						this.htmlMode && (s.has(t) || a.has(t)) && this.foreignContext.shift(),
						...
						this.htmlMode &&
							...
							null == (D = (o = this.cbs).onclosetag) || D.call(o, 'br', !1));
					else {
						let A = this.stack.indexOf(t);
						if (-1 !== A)
							for (let I = 0; I <= A; I++) {
								let Q = this.stack.shift();
								null == (B = (g = this.cbs).onclosetag) || B.call(g, Q, I !== A);
							}
						else this.htmlMode && 'p' === t && (this.emitOpenTag('p'), this.closeCurrentTag(!0));
					}
					this.startIndex = I + 1;
				}
				onselfclosingtag(A) {
				...
				closeCurrentTag(A) {
					...
					(this.endOpenTag(A),
						this.stack[0] === B &&
							(null == (g = (I = this.cbs).onclosetag) || g.call(I, B, !A), this.stack.shift()));
				}
				onattribname(A, I) {
					this.startIndex = A;
					let g = this.getSlice(A, I);
					...
					((this.endIndex = I),
						null == (Q = (g = this.cbs).onattribute) ||
							Q.call(
								...
								this.attribname,
								this.attribvalue,
								A === B.X.Double ? '"' : A === B.X.Single ? "'" : A === B.X.NoValue ? void 0 : null
							),
						this.attribs &&
				...
				oncomment(A, I, g) {
					var B, Q, C, E;
					((this.endIndex = I),
						null == (Q = (B = this.cbs).oncomment) || Q.call(B, this.getSlice(A, I - g)),
						null == (E = (C = this.cbs).oncommentend) || E.call(C),
						(this.startIndex = I + 1));
				}
				oncdata(A, I, g) {
					var B, Q, C, E, i, o, D, t, e, s;
					this.endIndex = I;
					...
					(!this.htmlMode || this.options.recognizeCDATA
						...
						: (null == (t = (D = this.cbs).oncomment) || t.call(D, `[CDATA[${a}]]`),
				...
				onend() {
					...
					if (this.cbs.onclosetag) {
						...
						for (let A = 0; A < this.stack.length; A++) this.cbs.onclosetag(this.stack[A], !0);
					}
				...
				getSlice(A, I) {
					for (; A - this.bufferOffset >= this.buffers[0].length; ) this.shiftBuffer();
					let g = this.buffers[0].slice(A - this.bufferOffset, I - this.bufferOffset);
					for (; I - this.bufferOffset > this.buffers[0].length; )
						(this.shiftBuffer(), (g += this.buffers[0].slice(0, I - this.bufferOffset)));
					return g;
				}
				shiftBuffer() {
				...
				write(A) {
					var I, g;
					if (this.ended) {
						null == (g = (I = this.cbs).onerror) || g.call(I, Error('.write() after done!'));
						return;
					}
					(this.buffers.push(A),
						this.tokenizer.running && (this.tokenizer.write(A), this.writeIndex++));
				}
				end(A) {
					var I, g;
					if (this.ended) {
						null == (g = (I = this.cbs).onerror) || g.call(I, Error('.end() after done!'));
						return;
					}
			...
			class a {
				constructor({ xmlMode: A = !1, decodeEntities: I = !0 }, g) {
					((this.cbs = g),
						...
						(this.entityDecoder = new D.Wf(A ? D.sr : D.qN, (A, I) => this.emitCodePoint(A, I))));
				}
				reset() {
					((this.state = i.Text),
						(this.buffer = ''),
				...
				stateSpecialStartSequence(A) {
					...
					if (I ? e(A) : (32 | A) === this.currentSequence[this.sequenceIndex]) {
						if (!I) return void this.sequenceIndex++;
					} else this.isSpecial = !1;
					((this.sequenceIndex = 0), (this.state = i.InTagName), this.stateInTagName(A));
				}
				stateInSpecialTag(A) {
					if (this.sequenceIndex === this.currentSequence.length) {
						if (A === E.Gt || t(A)) {
							...
							if (this.sectionStart < I) {
								let A = this.index;
								((this.index = I), this.cbs.ontext(this.sectionStart, I), (this.index = A));
							}
							((this.isSpecial = !1), (this.sectionStart = I + 2), this.stateInClosingTagName(A));
							return;
						}
						this.sequenceIndex = 0;
					}
				...
				fastForwardTo(A) {
					for (; ++this.index < this.buffer.length + this.offset; )
						if (this.buffer.charCodeAt(this.index - this.offset) === A) return !0;
				...
				stateInCommentLike(A) {
					A === this.currentSequence[this.sequenceIndex]
						...
						: 0 === this.sequenceIndex
							...
							: A !== this.currentSequence[this.sequenceIndex - 1] && (this.sequenceIndex = 0);
				}
				isTagStartChar(A) {
					return this.xmlMode
						? !e(A)
						: (A >= E.LowerA && A <= E.LowerZ) || (A >= E.UpperA && A <= E.UpperZ);
				}
				startSpecial(A, I) {
					((this.isSpecial = !0),
				...
				stateBeforeClosingTagName(A) {
					t(A) ||
						(A === E.Gt
							? (this.state = i.Text)
							: ((this.state = this.isTagStartChar(A) ? i.InClosingTagName : i.InSpecialComment),
								(this.sectionStart = this.index)));
				}
				...
				stateBeforeAttributeName(A) {
					A === E.Gt
						...
						: A === E.Slash
							...
							: t(A) || ((this.state = i.InAttributeName), (this.sectionStart = this.index));
				}
				stateInSelfClosingTag(A) {
					A === E.Gt
						? (this.cbs.onselfclosingtag(this.index),
							...
							(this.sectionStart = this.index + 1),
							(this.isSpecial = !1))
						: t(A) || ((this.state = i.BeforeAttributeName), this.stateBeforeAttributeName(A));
				}
				...
				handleInAttributeValue(A, I) {
					A === I || (!this.decodeEntities && this.fastForwardTo(I))
						? (this.cbs.onattribdata(this.sectionStart, this.index),
							(this.sectionStart = -1),
							this.cbs.onattribend(I === E.DoubleQuote ? o.Double : o.Single, this.index + 1),
							(this.state = i.BeforeAttributeName))
						: this.decodeEntities && A === E.Amp && this.startEntity();
				}
				...
				stateInEntity() {
					let A = this.entityDecoder.write(this.buffer, this.index - this.offset);
					A >= 0
						? ((this.state = this.baseState), 0 === A && (this.index = this.entityStart))
				...
				finish() {
					(this.state === i.InEntity && (this.entityDecoder.end(), (this.state = this.baseState)),
		...
		6570: function (A, I, g) {
			let B, Q, C, E;
			g.d(I, { P2: () => w });
			let i = (A, I) => I.some((I) => A instanceof I),
				o = new WeakMap(),
				D = new WeakMap(),
				t = new WeakMap(),
				e = {
					get(A, I, g) {
						if (A instanceof IDBTransaction) {
							if ('done' === I) return o.get(A);
							if ('store' === I)
								return g.objectStoreNames[1] ? void 0 : g.objectStore(g.objectStoreNames[0]);
						}
						return s(A[I]);
					},
					set: (A, I, g) => ((A[I] = g), !0),
					has: (A, I) => (A instanceof IDBTransaction && ('done' === I || 'store' === I)) || I in A
				};
			function s(A) {
				if (A instanceof IDBRequest) {
					...
					return (
						(I = new Promise((I, g) => {
							let B = () => {
									(A.removeEventListener('success', Q), A.removeEventListener('error', C));
								},
								Q = () => {
									(I(s(A.result)), B());
								},
							...
							(A.addEventListener('success', Q), A.addEventListener('error', C));
						})),
						t.set(I, A),
						I
					);
				...
				let I = (function (A) {
					if ('function' == typeof A)
						return (
							...
							? function (...I) {
									return (A.apply(a(this), I), s(this.request));
								}
							: function (...I) {
									return s(A.apply(a(this), I));
								};
					return (A instanceof IDBTransaction &&
						(function (A) {
							if (o.has(A)) return;
							let I = new Promise((I, g) => {
								let B = () => {
										(A.removeEventListener('complete', Q),
									...
									C = () => {
										(g(A.error || new DOMException('AbortError', 'AbortError')), B());
									};
								(A.addEventListener('complete', Q),
									A.addEventListener('error', C),
									A.addEventListener('abort', C));
							});
							o.set(A, I);
						})(A),
					i(A, B || (B = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])))
						? new Proxy(A, e)
						: A;
				})(A);
				return (I !== A && (D.set(A, I), t.set(I, A)), I);
			}
			let a = (A) => t.get(A);
			function w(A, I, { blocked: g, upgrade: B, blocking: Q, terminated: C } = {}) {
				let E = indexedDB.open(A, I),
					i = s(E);
				return (
					B &&
						E.addEventListener('upgradeneeded', (A) => {
							B(s(E.result), A.oldVersion, A.newVersion, s(E.transaction), A);
						}),
					g && E.addEventListener('blocked', (A) => g(A.oldVersion, A.newVersion, A)),
					i
						.then((A) => {
							(C && A.addEventListener('close', () => C()),
								Q && A.addEventListener('versionchange', (A) => Q(A.oldVersion, A.newVersion, A)));
						})
						.catch(() => {}),
					i
				);
			...
			let r = ['get', 'getKey', 'getAll', 'getAllKeys', 'count'],
				...
				c = new Map();
			function S(A, I) {
				if (!(A instanceof IDBDatabase && !(I in A) && 'string' == typeof I)) return;
				if (c.get(I)) return c.get(I);
				let g = I.replace(/FromIndex$/, ''),
					B = I !== g,
					Q = h.includes(g);
				if (!(g in (B ? IDBIndex : IDBObjectStore).prototype) || !(Q || r.includes(g))) return;
				let C = async function (A, ...I) {
					let C = this.transaction(A, Q ? 'readwrite' : 'readonly'),
						E = C.store;
					return (B && (E = E.index(I.shift())), (await Promise.all([E[g](...I), Q && C.done]))[0]);
				};
				return (c.set(I, C), C);
			}
			e = {
				...(C = e),
				get: (A, I, g) => S(A, I) || C.get(A, I, g),
				has: (A, I) => !!S(A, I) || C.has(A, I)
			};
			let G = ['continue', 'continuePrimaryKey', 'advance'],
				...
				R = {
					get(A, I) {
						if (!G.includes(I)) return A[I];
						let g = y[I];
						return (
							g ||
								(g = y[I] =
									function (...A) {
										N.set(this, n.get(this)[I](...A));
									}),
							g
						);
					}
			...
			async function* F(...A) {
				...
				let g = new Proxy(I, R);
				for (n.set(g, I), t.set(g, a(I)); I; )
					(yield g, (I = await (N.get(g) || I.continue())), N.delete(g));
			}
			function k(A, I) {
				return (
					(I === Symbol.asyncIterator && i(A, [IDBIndex, IDBObjectStore, IDBCursor])) ||
					('iterate' === I && i(A, [IDBIndex, IDBObjectStore]))
				);
			}
			e = {
				...(E = e),
				get: (A, I, g) => (k(A, I) ? F : E.get(A, I, g)),
				has: (A, I) => k(A, I) || E.has(A, I)
			};
		},
		1652: function (A, I, g) {
			g.d(I, { N: () => B });
		...
		3907: function (A, I, g) {
			...
			function C(A, I) {
				try {
					...
					let A,
						I = ((A = B.__externref_table_alloc()), B.__wbindgen_export_2.set(A, g), A);
					B.__wbindgen_exn_store(I);
				}
			}
			let E =
			...
			let i = null;
			function o() {
				return ((null === i || 0 === i.byteLength) && (i = new Uint8Array(B.memory.buffer)), i);
			}
			function D(A, I) {
			...
			function a(A, I, g) {
				...
				if (E !== B) {
					(0 !== E && (A = A.slice(E)), (Q = g(Q, B, (B = E + 3 * A.length), 1) >>> 0));
			...
			class S {
				...
				rewrite_js(A, I, g, Q) {
					let C = a(A, B.__wbindgen_malloc, B.__wbindgen_realloc),
						...
						o = t,
						D = a(g, B.__wbindgen_malloc, B.__wbindgen_realloc),
						e = t,
						s = B.rewriter_rewrite_js(this.__wbg_ptr, C, E, i, o, D, e, Q);
					if (s[2]) throw h(s[1]);
					return h(s[0]);
				}
				rewrite_js_bytes(A, I, g, Q) {
					let C,
						E =
							((C = (0, B.__wbindgen_malloc)(+A.length, 1) >>> 0),
							o().set(A, C / 1),
							(t = A.length),
							C),
						i = t,
						D = a(I, B.__wbindgen_malloc, B.__wbindgen_realloc),
						e = t,
						s = a(g, B.__wbindgen_malloc, B.__wbindgen_realloc),
						w = t,
						r = B.rewriter_rewrite_js_bytes(this.__wbg_ptr, E, i, D, e, s, w, Q);
					if (r[2]) throw h(r[1]);
					return h(r[0]);
				}
				constructor(A) {
					...
					if (I[2]) throw h(I[1]);
					return ((this.__wbg_ptr = I[0] >>> 0), c.register(this, this.__wbg_ptr, this), this);
				}
			}
			...
			function y() {
				...
				return (
					...
					(A.wbg.__wbindgen_string_get = function (A, I) {
						...
						var Q = null == g ? 0 : a(g, B.__wbindgen_malloc, B.__wbindgen_realloc),
							C = t;
...
((o.n = (A) => {
	...
	(o.d = (A, I) => {
		for (var g in I)
			o.o(I, g) && !o.o(A, g) && Object.defineProperty(A, g, { enumerable: !0, get: I[g] });
	}),
...
export {
	...
	q as errorTemplate,
	b as flagEnabled,
	x as getInjectScripts,
	m as getMostRestrictiveSite,
	W as getOwnPropertyDescriptorHandler,
	T as getPublicSuffixList,
	Z as getReferrerPolicy,
	V as getRewriter,
	O as getSiteDirective,
	P as handleFetch,
	j as htmlRules,
